...
func TestCORSMiddleware_AllowedOriginUsesExplicitOrigin(t *testing.T) {
	...
	r := gin.New()
	r.Use(CORSMiddleware(&config.Config{FrontendOrigins: "https://dashboard-procurement-demo.vercel.app"}))
	r.OPTIONS("/api/v1/auth/login", func(c *gin.Context) { c.Status(http.StatusNoContent) })

	req := httptest.NewRequest(http.MethodOptions, "/api/v1/auth/login", nil)
	req.Header.Set("Origin", "https://dashboard-procurement-demo.vercel.app")
	req.Header.Set("Access-Control-Request-Method", http.MethodPost)

	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)

	if got := rec.Header().Get("Access-Control-Allow-Origin"); got != "https://dashboard-procurement-demo.vercel.app" {
